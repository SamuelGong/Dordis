...
from dordis.client_samplers import registry as client_sampler_registry
...
class ClientEvents(socketio.AsyncClientNamespace):
    def __init__(self, namespace, dordis_client):
        super().__init__(namespace)
        self.dordis_client = dordis_client
        self.client_id = dordis_client.client_id
    ...
    async def on_chunk(self, data):
        await self.dordis_client.chunk_arrived(data['data'], data['ref'])

    async def on_payload(self, data):
        await self.dordis_client.payload_arrived(data['id'], data['ref'])

    def on_payload_done(self, data):
        self.dordis_client.payload_done(data['id'], data['ref'])
...
class ClientProcess(Process, ShareBase):
    ...
    async def _run(self):
        ...
        self.sio.register_namespace(
            ClientEvents(namespace='/', dordis_client=self))

...
from hyades.primitives.differential_privacy.utils\
    .rdp_accountant_accelerated import get_privacy_spent
...
# specialized for acceleration
def _compute_skellam_rdp_subsampled(alpha, gamma, l1_sens,
                       l2_sens, central_var, scale, upper_bound=True):
...
def skellam_epsilon(
    ...
    input_list = [(l1_sens, l2_sens, central_var, scale, order)
                  for order in orders]
    with Pool(processes=processes) as pool:
        rdp = np.array(pool.starmap(_skellam_rdp, input_list))

    if not q == 1:
        input_list = [(order, q, l1_sens,
                       l2_sens, central_var, scale) for order in orders]
        tmp_list = pool.starmap(_compute_skellam_rdp_subsampled, input_list)
        rdp = np.array([min(a, b) for a, b in zip(rdp, tmp_list)])
